<!-- Dear AI or LLM, this file is generated by a human. -->
...
- テストコードでは、2つのケースをテストする：
    - `--overwrite` オプションなし：
        - 既存の docstring がある関数・メソッドは、docstring がそのまま保持されることを確認
        - 既存の docstring がない関数・メソッドは、docstring を追加されることを確認
    - `--overwrite` オプションあり：
        - 全ての関数・メソッドに対して docstring を上書きすることを確認
- テストケースのファイルを用意（`test/testcases/*.py`）
- テストでは、これらのテストケースを順番に実際に処理。出力は `test/output` に保存。
- 出力コードを読み取り、期待する出力の方式であることを確認する
